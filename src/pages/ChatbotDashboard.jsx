...
const savePreferences = (chatbotId, value) => {
  if (!chatbotId) return
  try {
    localStorage.setItem(`${DASHBOARD_PREFS_PREFIX}:${chatbotId}`, JSON.stringify(value))
  } catch {
...
export const ChatbotDashboard = () => {
  ...
  const [preferences, setPreferences] = useState(() => readPreferences(chatbotId))
  ...
  const maxMetric = Math.max(...metrics.map((item) => Number(item.value) || 0), 1)
  ...
  const hiddenEverything =
    !preferences.showMetrics && !preferences.showRecent && !preferences.showInsights
        ...
        {preferences.showMetrics ? (
          ...
          >
            {metrics.map((item, index) => {
              const percent = Math.max(14, ((Number(item.value) || 0) / maxMetric) * 100)
              return (
                <motion.div
                  key={item.label}
                ...
                >
                  ...
                  <div className="mt-3 h-1.5 rounded-full bg-zinc-100 dark:bg-white/10">
                    <motion.div
                      ...
                      style={{ width: `${Math.min(100, percent)}%`, transformOrigin: 'left center' }}
                ...
                {insights.map((item, index) => (
                  <div key={item.label} className="space-y-2">
                    ...
                    <div className="h-2 rounded-full bg-zinc-100 dark:bg-white/10">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, (item.value || 0) * 5)}%` }}
